...
02/06 - MEJORAS IMPLEMENTADAS:
- Eliminado loop en reembolsoParcial()
- Optimizados calculos de porcentajes + reduccion gas
- Simplificar gestion de estado
- Agregar paginacion en consultas
- Implementados limite de gas
- Hacer mas eficientes las estructuras de datos
- manejar ofertas no deseadas
*/
...
contract Subasta {
	...
	// MEJORA matematica consumo gas
    uint256 public constant INCREMENTO_FACTOR = 105; 								// 5% = 105/100 
    uint256 public constant COMISION_FACTOR = 98;    								// 2% = 98/100

    // MEJORA escalabilidad de gas 
    uint256 public constant MAX_OFERTAS_CONSULTA = 5;
    uint256 public constant MAX_PARTICIPANTES = 20;
    ...
    // ===== EVENTOS  =====
    // MEJORA Eventos con menos parámetros
    ...
    modifier limitarParticipantes() {
        if (!yaParticipo[msg.sender]) {
            require(participantesActuales < MAX_PARTICIPANTES, "Cantidad Limite de participantes alcanzado");
            _;
        } else {
            _;
        }
    ...
    function ofertar() external payable soloActiva limitarParticipantes {
        ...
        uint256 montoMinimoRequerido = (mejorOferta * INCREMENTO_FACTOR) / 100;
        
        require(nuevoDepositoTotal > montoMinimoRequerido, "La oferta no supera el minimo (+5%)");
        ...
        // Crear nueva oferta (solo si necesario para historial)
        ofertas.push(Oferta({
    ...
    function mostrarTodasOfertas() external view returns (Oferta[] memory) {
        require(ofertas.length <= MAX_OFERTAS_CONSULTA, "Demasiadas ofertas (usar funcion mostrarOfertas)");
        return ofertas;
    }
    ...
    function devolverDepositos() external soloFinalizada {
        ...
        uint256 montoNeto = (montoADevolver * COMISION_FACTOR) / 100;
        ...
        (bool successComision, ) = payable(propietario).call{value: comision}("");	// preguntar 02-06
        require(successComision, "Error al enviar comision");
        
        emit DevolverDeposito(msg.sender, montoNeto, comision);
    }
    
    // MEJORA reembolsoParcial() SIN LOOP - OPTIMIZACION CRITICA
    ...
    function montoMinimoParaSuperar() external view returns (uint256) {
        ...
        return (mejorOferta * INCREMENTO_FACTOR) / 100;
    }
